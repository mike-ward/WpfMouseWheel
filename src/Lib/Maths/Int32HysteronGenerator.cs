...
namespace Logitech.Maths
{
    public class Int32HysteronGenerator
    {
        ...
        /// <param name="minEndpointWidth">
        ///     Identifies the minimum width of the left margin of the first template and the right
        ///     margin of the last template in normalized coordinates
        /// </param>
        /// <returns>A list of hysteron operators</returns>
        public static IEnumerable<Func<int, int>> CreateFunctions(int hysteronCount, int period, int interval = 1,
            int minHysteronWidth = 2, int minEndpointWidth = 1)
        {
            var pos = 0;
        ...
        /// <summary>
        ///     Computes, in normalized coordinates, the optimal list of adjacent windowed hysterons that can fit into a given
        ///     size.
        /// </summary>
        ...
        ///     The first identifies the width of the left margin (the length between the front edges of the window and the
        ///     hysteron loop)
        ///     The second identifies the width of the hysteron loop
        ///     The third identifies the width of the right margin (the length between the back edges of the hysteron loop and the
        ///     window)
        /// </remarks>
        private static IEnumerable<int[]> CreateTemplates(int patternSize, int hysteronCount, int minHysteronWidth,
            int minEndpointWidth)
        {
            ...
            {
                var templates = new int[hysteronCount][];
                for (var i = 0; i < hysteronCount; ++i) templates[i] = new[] { 0, hysteronWidth, 0 };
                // initialize left and right endpoints widths
                templates[0][0] = templates[hysteronCount - 1][2] = minEndpointWidth;
                if (hysteronWidth > minHysteronWidth)
                    for (var i = 0; i < hysteronCount; ++i)

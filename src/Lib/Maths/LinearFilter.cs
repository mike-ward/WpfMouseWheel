...
namespace Logitech.Maths
{
    ...
    public class DifferentialLinearFilter
    {
        ...
        public override string ToString() => $"t={_x,6:F3}, se={_se,6:F2}, ymax={_ymax,5:F2}, dyc={_dyc,5:F2}, ys={_ys,5:F2}";

        #endregion

        #region Helpers

        private double CoerceY(double y1) => Math.Sign(_ymax) > 0 ? Math.Min(_ymax, y1) : Math.Max(_ymax, y1);
        ...
        {
            if (DoubleEx.IsZero(dy1)) throw new ArgumentNullException(nameof(dy1));
            double sm;
            if (double.IsNaN(_x))
            {
                sm = smin;
